...
SHARED_FINAL_TEST_CHARTS_DIR = (SHARED_DAILY_HTML_DIR / CFG["OUTPUT"]["final_test_charts"])
SHARED_FIRST_PASS_CHARTS_DIR = (SHARED_DAILY_HTML_DIR / CFG["OUTPUT"]["first_pass_charts"])
...
def make_scope_filter_sql_excluding_565x(
    ...
    """
    Scope filter that excludes 5650 / 5651 / 5652 entirely.
    ...
    if station_value is not None:
        ...
        clauses.append(f"COALESCE(TRIM(CAST({h_prefix}station AS VARCHAR)), '') NOT IN ('5650','5651','5652')")
...
def get_lot_list_by_scope(device_code: str, station_value: str | None) -> list[str]:
    ...
    exclude_retest_filter = "AND d.station NOT IN ('5650','5651','5652')"
    query = f"""
...
def build_mg_unknown_exclusion_sql(
...
def get_top10_rpr_errcode_pareto_sql(
    ...
    if station_value is not None:
        ...
        filters.append(
            "COALESCE(TRIM(CAST(h.station AS VARCHAR)), '') NOT IN ('5650','5651','5652')"
        )
    ...
    unknown_exclusion_sql = build_mg_unknown_exclusion_sql(
...
def get_available_dashboard_scopes() -> list[dict]:
    ...
    for _, row in df.iterrows():
        ...
        if device_code in QX_DEVICES:
            seen_mg_devices.add(device_code)
            scopes.append({
    ...
    for mg_device in sorted(seen_mg_devices):
        scopes.append({
            "device_code": mg_device,
            "station": None,
            "tab_label": f"{mg_device}-OA",
            "section_label": f"{mg_device}-OA",
        })
...
def get_ranked_period_scopes() -> list[dict]:
    ...
    for _, row in work.iterrows():
        ...
        if device_code in QX_DEVICES:
            mg_family_rows.setdefault(device_code, {})
            mg_family_rows[device_code][station] = qty
        else:
            regular_rows.append({
                "device_code": device_code,
                "station": station,
    ...
    mg_family_rank_rows = []
    for mg_device, station_qty_map in mg_family_rows.items():
        mg_family_rank_rows.append({
            "device_code": mg_device,
            "station": None,
            "rank_qty": sum(station_qty_map.values()),
            "family_type": "mg_family",
        })

    rank_seed = sorted(
        regular_rows + mg_family_rank_rows,
    ...
    for item in rank_seed:
        ...
        if item["family_type"] == "regular":
            ...
            station_qty_map = mg_family_rows.get(device_code, {})
...
def is_mg_oa_scope(device_code: str, station_value: str | None) -> bool:
    dc = (device_code or "").strip().upper()
...
def build_excluded_retest_station_sql(
    ...
    return f"""
        AND NOT (
            COALESCE(TRIM(CAST({header_station_expr} AS VARCHAR)), '') IN ('5650', '5651', '5652')
...
def get_daily_summary_table_sql(device_code_value: str, station_value: str | None = None) -> str:
    ...
    if station_value is not None:
        ...
        filters.append("station NOT IN ('5650','5651','5652')")
...
def render_handler_site_7day_timeseries_charts(
    ...
    st.subheader("Top 10 Handler-Site 7-Day RPR Time Series")
...
def render_handler_top5_rpr_chart(df: pd.DataFrame, section_label: str, scope_key: str):
    ...
    fig.update_layout(
        ...
        font=dict(size=15, color="black"),
        xaxis=dict(
            title=dict(text="Handler", standoff=65),
            type="category",
            categoryorder="array",
            categoryarray=order,
            tickangle=-30
...
def render_handler_site_top5_rpr_chart(df: pd.DataFrame, section_label: str, scope_key: str):
    ...
    fig.update_layout(
        title=f"{section_label} — Top 10 Handler-Site vs Top 5 High RPR errCode<br><sup>Date Range: {date_scope_label}</sup>",
        autosize=True,
        height=820,
        barmode="stack",
        hovermode="y unified",
        ...
        legend=top_legend(),
        margin=dict(l=130, r=180, t=210, b=140)
    )
    ...
    if handler_site_7day_figs:
        ...
        for item in handler_site_7day_figs:
            ...
            inner_html += table_html(
                ...
                "Top 10 Handler-Site 7-Day RPR Time Series Data"
            )

        handler_site_7day_html = collapsible_html(
            "Show / Hide Top 10 Handler-Site 7-Day RPR Time Series",
            inner_html,
            open_default=False
        )
...
def build_rpr_html_report(
    ...
    if handler_site_7day_figs:
        ...
        handler_site_7day_html = "<h2>Top 10 Handler-Site 7-Day RPR Time Series</h2>"
