...
# Global template configuration with explicit automargin enabling
pio.templates["custom_black"].layout.update(
    font=dict(color="black"),
    margin=dict(l=120, r=130, t=170, b=190),
...
st.set_page_config(
    page_title="📊 USI Yield Dashboard",
    layout="wide"
)
...
def make_scope_filter_sql_excluding_565x(
    device_code: str,
    ...
    physical_device = get_physical_device_code(device_code)
    device_escaped = sql_safe(physical_device)
    ...
    if station_value is not None:
        station_escaped = sql_safe(str(station_value))
    ...
    elif is_QX_oa_scope(device_code, station_value):
        # QX1/QX2 Overall Assembly combines the three production stages.
        clauses.append(
            f"COALESCE("
            f"TRIM(CAST({h_prefix}station AS VARCHAR)), "
            f"'') IN ('1000', '1001', '1002')"
        ...
        # General overall scope: remove only dedicated retest stations.
        clauses.append(
            f"COALESCE("
            f"TRIM(CAST({h_prefix}station AS VARCHAR)), "
            f"'') NOT IN ('1010', '1011', '1012')"
        )

    split_sql = build_device_type_split_sql(
        device_code,
        header_alias
    ).strip()

    if split_sql.startswith("AND "):
        clauses.append(split_sql[4:])
...
def make_scope_filter_sql(
    ...
    physical_device = get_physical_device_code(device_code)
    device_escaped = physical_device.replace("'", "''")

    clauses = [
        f"COALESCE(TRIM(CAST({h_prefix}device_code AS VARCHAR)), '') = '{device_escaped}'"
    ]
    ...
    split_sql = build_device_type_split_sql(device_code, header_alias).strip()
    if split_sql.startswith("AND "):
        clauses.append(split_sql[4:])
...
@st.cache_data(show_spinner=False)
def get_lot_list_by_scope(
    device_code: str,
    station_value: str | None
) -> list[str]:

    scope_filter = make_scope_filter_sql_excluding_565x(
        device_code,
        station_value
    )
    ...
    query = f"""
        SELECT DISTINCT
            TRIM(CAST(schedule_no AS VARCHAR)) AS schedule_no
        FROM {HEADER_TABLE}
        {scope_filter}
...
def build_QX_unknown_exclusion_sql(
...
def build_period_yield_export_df(plot_df: pd.DataFrame) -> pd.DataFrame:
    if plot_df is None or plot_df.empty:
        return pd.DataFrame()

    export_df = plot_df[[
        "x_label", "Test-In QTY", "Final Output", "1st Yield", "FTY"
    ]].copy()
...
def show_clean_table(df: pd.DataFrame, height: int = 320):
    if df is None or df.empty:
        return
...
def get_target_legend_label(metric_name: str, target_value: float, target_info: dict) -> str:
    mode = str(target_info.get("limit_mode", "")).lower()

    if mode == "manual":
        return f"{metric_name} Target ({target_value:.2f}%)"

    return f"3-sig {metric_name} Target ({target_value:.2f}%)"

def get_lrr_trigger_label(target_info: dict, target_value: float) -> str:
    mode = str(target_info.get("limit_mode", "")).lower()

    if mode == "manual":
        return f"LRR Trigger: FTY Target ({target_value:.2f}%)"

    return f"LRR Trigger: FTY Target (3-sig LCL @ {target_value:.2f}%)"

def clean_errcode_rows_and_apply_trend(
    df: pd.DataFrame,
    metric_rows: list[str]
) -> pd.DataFrame:
    """
    For Top1–Top5 errCode and Other errCodes rows:

    1. Remove errCode / soft-bin description from date-column cells.
       Example:
           0.25% 101089
           PARAM 234
       becomes:
    ...
    2. Add RPR/defect-style trend indicators:
       Increase = red ▲
       Decrease = green ▼
       No change = gray ◆

    The Metric column remains unchanged.
    """
    if df is None or df.empty or "Metric" not in df.columns:
        return df
    ...
    valid_metric_rows = [
        ...
        if metric in out["Metric"].astype(str).tolist()
    ]

    if not valid_metric_rows:
        return out

    # Remove errCode and soft-bin text from all date/data columns.
    for col in out.columns:
        if col == "Metric":
            continue

        row_mask = out["Metric"].isin(valid_metric_rows)

        out.loc[row_mask, col] = out.loc[row_mask, col].apply(
            keep_pct_only
        )

    # Higher defect/RPR is bad:
    # UP = red, DOWN = green, SAME = gray.
    out = apply_rpr_trend_arrows_to_metric_rows(
        out,
        valid_metric_rows
    )
...
def get_monthly_lot_target_source_df(device_code: str, station_value: str | None) -> pd.DataFrame:
    ...
    exclude_retest_filter = "AND d.station NOT IN ('1000','1001','1002')"
...
def get_scope_target_limits_from_back_months(
    ...
    if KPI_LIMIT_MODE == "manual" and device_code is not None:
        return get_manual_scope_target_limits(device_code, station_value)
...
def get_kpi_target_labels_html(
    ...
    is_manual_mode = str(target_info.get("limit_mode", "")).lower() == "manual"

    if is_manual_mode:
        source_label = "Manual Config"
        target_label = "Manual Limit"
        back_month_line = f"Target Source: {source_label}"
    else:
        source_month = target_info.get("target_source_month_label") or "-"
        target_label = "3-sig LCL"
        back_month_line = f"Back Month Used for IQR Target Calc: {source_month}"

    if target_info["no_back_month_data"]:
        return f"""
        <div style="font-size:13px; color:#666; margin-bottom:10px; line-height:1.6;">
            FTY Target ({target_label}): 98.00%<br>
            FPY Target ({target_label}): 95.00%<br>
            LRR Hold Trigger: FTY Target ({target_label}): 98.00%<br>
            RPR Threshold: &lt; 2%<br>
    ...
    fty_text = f"{float(fty_raw):.2f}%" if fty_raw is not None else "98.00%"
    fpy_text = f"{float(fpy_raw):.2f}%" if fpy_raw is not None else "95.00%"

    return f"""
    <div style="font-size:15px; color:#666; margin-bottom:10px; line-height:1.6;">
        {back_month_line}<br>
        FTY Target ({target_label}): {fty_text}<br>
        FPY Target ({target_label}): {fpy_text}<br>
        LRR Hold Trigger: FTY Target ({target_label}): {fty_text}<br>
        RPR Threshold: &lt; 2%
    </div>
    """
...
def get_top10_rpr_errcode_pareto_sql(
    ...
    if is_QX_oa_scope(device_code_value, station_value):
        filters.append(
            "COALESCE("
            "TRIM(CAST(h.station AS VARCHAR)), "
            "'') IN ('1000', '1001', '1002')"
        )
        ...
        filters.append(
            f"COALESCE("
            f"TRIM(CAST(h.station AS VARCHAR)), "
            f"'') = '{sql_safe(str(station_value))}'"
        )
    ...
    unknown_exclusion_sql = build_QX_unknown_exclusion_sql(
...
def get_manual_scope_target_limits(device_code: str, station_value: str | None = None) -> dict:
    fty_limit = get_manual_fty_limit_pct(device_code, station_value)
    fpy_limit = max(0.00, round(fty_limit - 2.00, 4))
...
def is_virtual_mq1_ref1(device_code: str) -> bool:
    return str(device_code or "").strip().upper() == "MQ1_REF1"


def get_physical_device_code(device_code: str) -> str:
    dc = str(device_code or "").strip().upper()
    if dc == "MQ1_REF1":
        return "MQ1"
    return dc
...
def build_device_type_split_sql(
    ...
    h = f"{header_alias}." if header_alias else ""
    dc = str(device_code or "").strip().upper()

    if dc == "MQ1_REF1":
        return f"""
            AND UPPER(COALESCE(TRIM(CAST({h}device_type AS VARCHAR)), '')) LIKE '%REF1%'
        """

    if dc == "MQ1":
        return f"""
            AND UPPER(COALESCE(TRIM(CAST({h}device_type AS VARCHAR)), '')) NOT LIKE '%REF1%'
        """
...
def scope_has_header_data_on_date(device_code: str, station_value: str | None, target_day: pd.Timestamp) -> bool:
    scope_filter = make_scope_filter_sql_excluding_565x(device_code, station_value)
...
def get_available_dashboard_scopes() -> list[dict]:
    ...
    df["device_code"] = df.apply(
        lambda r: "MQ1_REF1"
        if str(r["device_code"]).strip().upper() == "MQ1"
        and "REF1" in str(r.get("device_type", "")).upper()
        else str(r["device_code"]).strip(),
        axis=1
    )

    df = df[["device_code", "station"]].drop_duplicates()
    ...
    for _, row in df.iterrows():
        ...
        display_label = display_device_name(tab_label)

        scopes.append({
            "device_code": device_code,
            "station": station,
            "tab_label": display_label,
            "section_label": display_label,
        })
...
def get_ranked_period_scopes() -> list[dict]:
    ...
    query = f"""
        ...
        FROM base
        GROUP BY device_code, device_type, station
    """
    ...
    work["device_type"] = work["device_type"].astype(str).str.strip()
    work["station"] = work["station"].astype(str).str.strip()
    work["past_month_output_qty"] = pd.to_numeric(
        work["past_month_output_qty"],
        errors="coerce"
    ).fillna(0)

    # Split MQ1 into MQ1 and MQ1_REF1 for top-level YoY/QoQ/MoM charts
    work["device_code"] = work.apply(
        lambda r: "MQ1_REF1"
        if str(r["device_code"]).strip().upper() == "MQ1"
        and "REF1" in str(r.get("device_type", "")).upper()
        else str(r["device_code"]).strip(),
    ...
    # Re-aggregate after virtual MQ1_REF1 split
    work = (
        work.groupby(["device_code", "station"], as_index=False)
        .agg(past_month_output_qty=("past_month_output_qty", "sum"))
    )
...
def build_excluded_retest_station_sql(
    ...
    flow_expr: str | None = None
) -> str:
    """
    Exclude only dedicated retest stations:
    1010 / 1011 / 1012.

    Do not remove RT1 / RT2 detail rows from normal production
    stations 1000 / 1001 / 1002 because those rows are required
    for RPR recovery calculations.
    """
    _ = flow_expr

    retest_station_sql = ", ".join(
        f"'{station}'"
        for station in EXCLUDED_RETEST_STATIONS
    )

    return f"""
        AND COALESCE(
            TRIM(CAST({header_station_expr} AS VARCHAR)),
            ''
        ) NOT IN ({retest_station_sql})
    """
...
def get_daily_summary_table_sql(device_code_value: str, station_value: str | None = None) -> str:
    ...
    if station_value is not None:
        ...
        filters.append("station NOT IN ('1000','1001','1002')")
...
def build_4week_fty_errcode_chart_df(
    ...
    if defect_reference_df.empty:
        ...
        if is_QX_rf_scope:
            # Full available 4-week window for MG RF only.
...
def build_handler_site_7day_rpr_timeseries_df(
    ...
    def normalize_handler(value) -> str:
        if pd.isna(value):
            return ""
    ...
    normalized_handler_value = normalize_handler(handler_value)
    normalized_site_value = str(site_value).strip()

    work = work[
        (work["handler"] == normalized_handler_value) &
        (work["site"] == normalized_site_value)
    ].copy()
...
def render_handler_site_7day_timeseries_charts(
    ...
    for idx, row in top10_ordered.iterrows():
        ...
        # Convert existing x_label like 04/18 back to full date using current year
        ...
        max_day = plot_df["plot_date"].max().normalize()
        min_day = max_day - pd.Timedelta(days=6)

        full_days = pd.DataFrame({
            "plot_date": pd.date_range(
                min_day,
                max_day,
                freq="D"
            )
        ...
        plot_df["plot_date"] = plot_df["plot_date"].dt.normalize()

        plot_df = full_days.merge(
            plot_df,
            on="plot_date",
            how="left"
        )
        ...
        fig.add_trace(
            go.Scatter(
                ...
                y=[2] * len(plot_df),
                mode="lines",
                name="RPR Threshold (2%)",
                line=dict(
                    color="black",
                    width=2,
                    dash="dash"
                ),
                hovertemplate="RPR Threshold: 2%<extra></extra>"
            )
        )
        fig.add_trace(
            go.Scatter(
...
def build_period_yield_export_df(plot_df: pd.DataFrame) -> pd.DataFrame:
    ...
    display_df = export_df.set_index("Period").T.reset_index().rename(
...
def render_yoy_scope_section(device_code: str, station_value: str | None, section_label: str):
    ...
    defect_fig, defect_table_df = render_period_fty_errcode_chart(
        ...
        section_label,
        "YoY",
        chart_key=f"YoY_{device_code}_{station_value or 'OA'}"
    )
...
def render_qoq_scope_section(device_code: str, station_value: str | None, section_label: str):
    ...
    defect_fig, defect_table_df = render_period_fty_errcode_chart(
        ...
        section_label,
        "QoQ",
        chart_key=f"QoQ_{device_code}_{station_value or 'OA'}"
    )
...
def render_mom_scope_section(device_code: str, station_value: str | None, section_label: str):
    ...
    defect_fig, defect_table_df = render_period_fty_errcode_chart(
        ...
        section_label,
        "MoM",
        chart_key=f"MoM_{device_code}_{station_value or 'OA'}"
    )
...
def render_mother_lot_yield_trend(
    ...
    fig.update_layout(
        ...
        dragmode="zoom",
        xaxis=dict(
            title=dict(text="Mother Lot", standoff=65),
            type="category",
            categoryorder="array",
            categoryarray=x_order,
            tickangle=-30
    ...
    render_plotly_fixed(
        fig,
        key=f"mother_lot_yield_{scope_key}"
    )
...
def render_station_4week_yield_trend(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="Final Yield",
            yaxis="y2",
            line=dict(color=FTY_GREEN, width=3),
            marker=dict(color=FTY_GREEN, size=8),
    ...
    render_plotly_fixed(
        fig,
        key=f"row1_yield_trend_{scope_key}"
    )
...
def render_daily_summary_fty_errcode_chart(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="FTY",
            yaxis="y2",
            line=dict(color=FTY_GREEN, width=4),
            marker=dict(color=FTY_GREEN, size=9),
    ...
    fig.add_trace(
        go.Scatter(
            ...
            textfont=dict(
                color=[
                    "#C00000" if float(v) > 1 else "black"
    ...
    render_plotly_fixed(
        fig,
        key=f"row2_defect_distribution_{scope_key}"
    )
    ...
    err_metric_rows = [
        metric
        for metric in display_df["Metric"].astype(str).tolist()
        if metric.startswith("top") and "_errCode" in metric
    ]

    err_metric_rows.append("other_errCodes")

    display_df = clean_errcode_rows_and_apply_trend(
        display_df,
        err_metric_rows
    )
...
def render_top10_rpr_errcode_pareto(df: pd.DataFrame, section_label: str, scope_key: str):
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title=dict(text="Retest Rate", standoff=45),
            side="left",
            range=[auto_rr_min, auto_rr_max]
        ),
        yaxis2=dict(
            title=dict(text="Overall Retest Rate %", standoff=45),
            overlaying="y",
            side="right",
            showgrid=False,
            range=[auto_rr2_min, auto_rr2_max]
        ),
        legend=top_legend(),
        margin=dict(l=120, r=130, t=210, b=190)
    )
    ...
    render_plotly_fixed(
        fig,
        key=f"row5_top5_rpr_errcode_{scope_key}"
    )
...
def render_handler_top5_rpr_chart(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            textfont=dict(
                color=[
                    "#C00000" if float(v) > 2 else "black"
                    for v in plot_df["total_rpr_pct"]
                ],
                size=11
            ),
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            range=[
                x_min,
                auto_x_max if x_max == 100 else x_max
            ]
        ),
        yaxis=dict(
            title=dict(
    ...
    render_plotly_fixed(
        fig,
        key=f"row6_handler_rpr_{scope_key}"
    )
...
def render_handler_site_top5_rpr_chart(df: pd.DataFrame, section_label: str, scope_key: str):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            hovertemplate="RPR Target: 2%<extra></extra>",
            cliponaxis=False
        )
    )
    ...
    render_plotly_fixed(
        fig,
        key=f"row7_handler_site_rpr_{scope_key}"
    )
...
def build_station_html_report(
    ...
    <head>
        <meta charset="utf-8">
        <title>📊 USI Yield Dashboard - {station_label}</title>
    ...
    <body>
        <h1>📊 USI Yield Dashboard - {station_label}</h1>
...
def build_final_test_html_report(
    ...
    <head>
        <meta charset="utf-8">
        <title>USI Daily Yield - {station_label}</title>
        <script src="https://cdn.plot.ly/plotly-2.35.2.min.js"></script>
        <style>
            body {{
                font-family: Arial, sans-serif;
    ...
    <body>
        <h1>📊 USI Daily Yield - {station_label}</h1>
        <p>Exported from Streamlit dashboard</p>
...
def build_rpr_html_report(
    ...
    <head>
        <meta charset="utf-8">
        <title>USI RPR Data - {station_label}</title>
        <script src="https://cdn.plot.ly/plotly-2.35.2.min.js"></script>
        <style>
            body {{
                font-family: Arial, sans-serif;
    ...
    <body>
        <h1>📊 USI RPR Data - {station_label}</h1>
        <p>Exported from Streamlit dashboard</p>
...
def build_period_html_report(
    ...
    <head>
        <meta charset="utf-8">
        <title>📊 USI {period_label} Dashboard - {section_label}</title>
        <script src="https://cdn.plot.ly/plotly-2.35.2.min.js"></script>
        <style>
            body {{
                font-family: Arial, sans-serif;
    ...
    <body>
        <h1>📊 USI {period_label} Dashboard - {section_label}</h1>
...
def build_period_tab_html_report(period_label: str, rendered_sections: list[dict]) -> str:
    ...
    <head>
        <meta charset="utf-8">
        <title>📊 USI {period_label} Dashboard</title>
        <script src="https://cdn.plot.ly/plotly-2.35.2.min.js"></script>
        <style>
            body {{
                font-family: Arial, sans-serif;
    ...
    <body>
        <h1>📊 USI {period_label} Dashboard</h1>
        {''.join(body_sections)}
            <script>
...
def render_scope_section(device_code: str, station_value: str | None, section_label: str):
    ...
    try:
        ...
        if not html_day_export_df.empty and "Metric" in html_day_export_df.columns:
            qty_mask = html_day_export_df["Metric"].isin(["Test-In QTY", "Final Output"])

            for col in html_day_export_df.columns:
                if col != "Metric":
                    html_day_export_df.loc[qty_mask, col] = pd.to_numeric(
                        html_day_export_df.loc[qty_mask, col],
                        errors="coerce"
        ...
        final_test_filename = f"USI_{export_file_stub}_Daily_Yield_{date_stub}.html"
        rpr_filename = f"USI_{export_file_stub}_RPR_data_{date_stub}.html"
...
# st.markdown(target_labels_html, unsafe_allow_html=True)
    ...
    fig = fix_plotly_axis_label_spacing(fig)
    period_scope_key = (
        f"{device_code}_{station_value or 'OA'}_{period_label}"
        .replace(" ", "_")
        .replace("/", "_")
    )
    ...
    render_plotly_fixed(
        fig,
        key=f"period_yield_{period_scope_key}"
    )

    return fig

def render_period_fty_errcode_chart(
    plot_df: pd.DataFrame,
    section_label: str,
    period_label: str,
    chart_key: str
    ...
    fig.add_trace(
        go.Scatter(
            ...
            textposition="top center",
            textfont=dict(
                color=["#C00000" if float(v) > 1 else "black" for v in total_fail_pct],
                size=11
            ),
            cliponaxis=False,
            hoverinfo="skip",
    ...
    render_plotly_fixed(
        fig,
        key=f"period_defect_{chart_key}"
    )
    ...
    is_QX_oa_section = str(section_label).strip().upper() in {"QX1-OA", "QX2-OA"}

    def fix_QX_oa_date_cell(value):
        if pd.isna(value):
            return value
...
# =========================================================
def main():
    st.title("📊 USI Yield Dashboard")
    st.caption("🦆 DuckDB + Streamlit for USI text-summary transformed data")
    ...
    if AUTO_EXPORT_MODE:
        ...
        # Also clear nested daily chart folders
        cleanup_folder_contents(SHARED_FINAL_TEST_CHARTS_DIR, patterns=["*.html"])
        cleanup_folder_contents(SHARED_FIRST_PASS_CHARTS_DIR, patterns=["*.html"])
    ...
    with top_tabs[0]:
        ...
        if not ranked_period_scopes:
            ...
            yoy_shared_path = SHARED_TOP_LEVEL_HTML_DIR / "USI_YoY_Dashboard.html"
            ...
            st.download_button(
                ...
                file_name="USI_YoY_Dashboard.html",
                mime="text/html",
    ...
    with top_tabs[1]:
        ...
        if not ranked_period_scopes:
            ...
            ensure_folder(SHARED_TOP_LEVEL_HTML_DIR)
            qoq_shared_path = SHARED_TOP_LEVEL_HTML_DIR / "USI_QoQ_Dashboard.html"
            ...
            st.download_button(
                ...
                file_name="USI_QoQ_Dashboard.html",
                mime="text/html",
    ...
    with top_tabs[2]:
        ...
        if not ranked_period_scopes:
            ...
            ensure_folder(SHARED_TOP_LEVEL_HTML_DIR)
            mom_shared_path = SHARED_TOP_LEVEL_HTML_DIR / "USI_MoM_Dashboard.html"
            ...
            st.download_button(
                ...
                file_name="USI_MoM_Dashboard.html",
                mime="text/html",
